...
{
    IBOutlet UIImageView *_horizFilterView;
...
- (void)viewDidLoad
{
    ...
    [_btnHot setBackgroundImage:[UIImage imageNamed:@"hottestSelectedbg"] forState:UIControlStateSelected];
    [_btnNew setBackgroundImage:[UIImage imageNamed:@"hottestSelectedbg"] forState:UIControlStateSelected];
    [_btnFilter setBackgroundImage:[UIImage imageNamed:@"categroySbg"] forState:UIControlStateSelected];
    ...
    [_btnHot setTitle:[[I18NControl bundle] localizedStringForKey:@"hottest" value:nil table:nil] forState:UIControlStateNormal];
    [_btnFilter setTitle:[[I18NControl bundle] localizedStringForKey:@"category" value:nil table:nil] forState:UIControlStateNormal];
    [_btnNew setTitle:[[I18NControl bundle] localizedStringForKey:@"lastest" value:nil table:nil] forState:UIControlStateNormal];
    ...
    [self.splitVC.detailViewController addChildViewController: hotProductVC];
    
    [self.splitVC.detailViewController.view addSubview:hotProductVC.view];
    
    [[NSNotificationCenter defaultCenter] addObserver:self selector:@selector(notificationFilterClick) name:CATEGORY_CLICK object:nil];
    [[NSNotificationCenter defaultCenter] addObserver:self selector:@selector(closeDetailVC) name:CLOSE_MASKVIEW object:nil];
}

-(void) viewWillAppear:(BOOL)animated
{
    //todo: add product filter here
    NSString * filterCategoryID = [AppController sharedInstance].filterCategoryID;
    if(filterCategoryID != nil)
    {
        [_arrayFilterConditions replaceObjectAtIndex:0 withObject:filterCategoryID];
    }
...
-(void)viewDidDisappear:(BOOL)animated
{
    ...
    //todo:delete filter here
    [AppController sharedInstance].filterCategoryID= nil;
...
{
    /*
     FLProductDetailViewController *detailVC = [[FLProductDetailViewController alloc] initWithNibName:@"ProductDetailViewController" bundle:nil];
     ...
     */
    [[NSNotificationCenter defaultCenter] postNotificationName:HOTVIEWCONTROLLER_CLOSE_MASKVIEW object:nil];
    ...
    productDetailVC.productInfo = product;
    
    [hotProductVC addChildViewController: productDetailVC];
    ...
    detailView=productDetailVC.view;
    detailView.frame=(CGRect){-1,0,447,748};
    
    [_maskView addSubview:detailView];
    [hotProductVC.view  addSubview:_maskView];
...
- (IBAction)filterClick:(id)sender
{
    ...
    [_btnFilter setBackgroundImage:[UIImage imageNamed:@"categroySbg"] forState:UIControlStateNormal];
    [_btnNew setBackgroundImage:[UIImage imageNamed:@"filterbarbg"] forState:UIControlStateNormal];
     [_btnHot setBackgroundImage:[UIImage imageNamed:@"filterbarbg"] forState:UIControlStateNormal];
    // test
    //    //如果开始没有取到菜单数据在这里重新获取
...
//    BOOL selectionIsExpand= (_horizFilterView.frame.origin.y >= _viewFilter.frame.origin.y+_viewFilter.frame.size.height ) ? YES : NO;
    ...
    if(selectionIsExpand){
        [UIView animateWithDuration:.5 animations:^{
            ...
            frame.origin.y = _viewFilter.frame.origin.y -frame.size.height;
            _horizFilterView.frame = frame;
            // 产品列表显示
            frame=_tableView.frame;
            frame.origin.y = _viewFilter.frame.origin.y + _viewFilter.frame.size.height;
        ...
        [UIView animateWithDuration:.5 animations:^{
            ...
            // 产品列表显示
            frame=_tableView.frame;
            frame.origin.y = _horizFilterView.frame.origin.y + _horizFilterView.frame.size.height;
            //frame.size.height -= _horizFilterView.frame.size.height;
            _tableView.frame = frame;
...
- (IBAction)hotClick:(id)sender
{
    ...
    [_btnFilter setBackgroundImage:[UIImage imageNamed:@"filterbarbg"] forState:UIControlStateNormal];
    [_btnNew setBackgroundImage:[UIImage imageNamed:@"filterbarbg"] forState:UIControlStateNormal];
    [_btnHot setBackgroundImage:[UIImage imageNamed:@"hottestSelectedbg"] forState:UIControlStateNormal];
...
- (IBAction)newChick:(id)sender
{
    ...
    [_btnFilter setBackgroundImage:[UIImage imageNamed:@"filterbarbg"] forState:UIControlStateNormal];
    [_btnNew setBackgroundImage:[UIImage imageNamed:@"hottestSelectedbg"] forState:UIControlStateNormal];
    [_btnHot setBackgroundImage:[UIImage imageNamed:@"filterbarbg"] forState:UIControlStateNormal];
...
-(void) initHorizFilterView
{
    ...
    [arrayAllCategory enumerateObjectsUsingBlock:^(id obj, NSUInteger idx, BOOL *stop) {
        ...
        filterMenuAll.categoryName = [[I18NControl bundle] localizedStringForKey:@"all" value:nil table:nil];
        ...
        // 加载筛选列表
        HorizFilter *horizFilter = [[HorizFilter alloc] initWithFrame:CGRectMake(0, (HorizFilterHeight+1)*idx, self.view.bounds.size.width, HorizFilterHeight)];
...
//        [_horizFilterView addSubview:lblHead];
    ...
    NSLog(@"高度是 =%f", frameFilterView.size.height);
    /*
...
-(void) closeDetailVC
{
    ...
    [UIView animateWithDuration:.2 delay:0 options:UIViewAnimationOptionCurveEaseOut animations:^{
        ...
        //        [productDetailVC.view removeFromSuperview];
