...
- (IBAction) sendPost:(id)sender
{
    ...
    if(_arrayImage.count > 4){
        [[[iToast makeText:@"最多允许上传四张图片！"] setGravity:iToastGravityCenter] showInView:self.view];
        return;
    }
    if (self.postType == AddPost || self.postType == EditPost)
    {
        if(_txtTitle.text.length==0){
            [[[iToast makeText:@"请输入贴子标题"] setGravity:iToastGravityCenter] showInView:self.view];
            return;
        }
        if(_txtTitle.text.length > MaxTitleLength){
            [[[iToast makeText:[NSString stringWithFormat:@"标题最多允许提交%d个字符", MaxTitleLength]] setGravity:iToastGravityCenter] showInView:self.view];
            return;
        }
    }
    if(_txtContent.text.length==0){
        [[[iToast makeText:@"请输入内容"] setGravity:iToastGravityCenter] showInView:self.view];
        return;
    }
    if(_txtContent.text.length > MaxContentLength){
        [[[iToast makeText:[NSString stringWithFormat:@"最多允许提交%d个字符", MaxContentLength]] setGravity:iToastGravityCenter] showInView:self.view];
        return;
    }
